...
const OwnerPaymentsModal = ({ isOpen, onClose, vehicle }) => {
  ...
  const fetchPayments = async () => {
    ...
    setIsLoading(true);
    try {
      const { data } = await api.get(`/owner-payments?vehicle_id=${vehicle.id}`);
      setPayments(data || []);
    } catch (e) {
  ...
  // ConfirmDeleteModal handles the API call now
  // Keeping this comment block instead of handleDelete just to preserve component structure
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Owner Payments: ${vehicle.vehicle_number}`} size="xl">
                ...
                payments.map(p => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>
                        <span className={`badge badge-${p.status === 'paid' ? 'success' : p.status === 'pending' ? 'warning' : 'info'}`}>
                          {p.status}
                        </span>
                      </td>
      ...
      <Modal isOpen={activeAction !== null} onClose={() => { setActiveAction(null); setEditId(null); }} title={activeAction === 'edit' ? 'Edit Payment' : 'Record Payment'} size="md">
        <OwnerPaymentForm 
          ...
          defaultAmount={vehicle.hired_details?.monthly_amount}
        />
      </Modal>
