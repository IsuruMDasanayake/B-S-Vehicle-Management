...
const DriverAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setDrivers(response.data.data.top_drivers || []);
    } catch (error) {
      console.error('Failed to fetch driver analytics');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div>
      <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)' }}>Detailed performance metrics for all drivers.</p>
        </div>
        ...
        <table className="table" style={{ width: '100%', textAlign: 'left', minWidth: '600px' }}>
          <thead>
            <tr>
              ...
              <th style={{ paddingBottom: '0.75rem', borderBottom: '1px solid var(--border-color)' }}>Driver Name</th>
              ...
              filteredDrivers.map((driver, idx) => (
                <tr key={idx}>
                  ...
                  <td style={{ padding: '0.75rem 0', borderBottom: '1px solid var(--border-color)', fontWeight: '500' }}>
                    Rs {driver.total_km > 0 ? (driver.net_revenue / driver.total_km).toFixed(2) : 0}
                  </td>
                </tr>
              ))
            ) : (
              <tr><td colSpan="6" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>No drivers found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
