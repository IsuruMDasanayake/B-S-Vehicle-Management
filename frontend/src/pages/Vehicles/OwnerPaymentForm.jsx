...
const schema = z.object({
  ...
  status: z.enum(['paid', 'pending', 'ongoing']),
  payment_date: z.string().optional().nullable(),
  notes: z.string().optional(),
});

const OwnerPaymentForm = ({ vehicleId, editId, onSuccess, onClose, defaultAmount }) => {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    ...
    defaultValues: { status: 'paid', amount: defaultAmount || '' },
  });

  useEffect(() => {
    if (isEditMode) {
      api.get(`/owner-payments/${editId}`)
        .then(({ data }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        await api.post(`/owner-payments/${editId}`, payload, { headers: { 'Content-Type': 'multipart/form-data' }});
        toast.success('Payment updated');
      } else {
        await api.post('/owner-payments', payload, { headers: { 'Content-Type': 'multipart/form-data' }});
