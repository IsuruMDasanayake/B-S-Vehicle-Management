...
const SparePartForm = ({ editId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/spare-parts/${id}`).then(({ data }) => {
        ...
        reset(data);
      }).catch(() => { toast.error('Failed to load'); if (!isModal) navigate('/vehicle/admin/spare-parts'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/spare-parts');
    } catch (e) { toast.error(e.response?.data?.message || 'Error'); }
    finally { setIsLoading(false); }
  };

  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/spare-parts'); };
