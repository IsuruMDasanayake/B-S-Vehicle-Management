...
const AccidentForm = ({ editId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/accidents/${id}`).then(({ data }) => {
        ...
        reset(data);
      }).catch(() => { toast.error('Failed to load'); if (!isModal) navigate('/vehicle/admin/accidents'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/accidents');
  ...
  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/accidents'); };
