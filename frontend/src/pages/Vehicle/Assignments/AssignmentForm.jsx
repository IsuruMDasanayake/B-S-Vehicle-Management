...
const AssignmentForm = ({ editId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/assignments/${id}`)
        ...
        .catch(() => { toast.error('Failed to load assignment'); if (!isModal) navigate('/vehicle/admin/assignments'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/assignments');
  ...
  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/assignments'); };
