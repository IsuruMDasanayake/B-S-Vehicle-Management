...
const RevenueLicenseForm = ({ editId, onSuccess, onClose, defaultVehicleId }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/revenue-licenses/${id}`).then(({ data }) => {
        ...
        reset(data);
      }).catch(() => { toast.error('Failed to load'); if (!isModal) navigate('/vehicle/admin/revenue-licenses'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/revenue-licenses');
    } catch (e) { toast.error(e.response?.data?.message || 'Error'); }
    finally { setIsLoading(false); }
  };

  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/revenue-licenses'); };
