...
const InsuranceForm = ({ editId, onSuccess, onClose, defaultVehicleId }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/insurance-policies/${id}`)
        ...
        .catch(() => { toast.error('Failed to load policy'); if (!isModal) navigate('/vehicle/admin/insurance'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/insurance');
    } catch (e) { toast.error(e.response?.data?.message || 'Something went wrong'); }
    finally { setIsLoading(false); }
  };

  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/insurance'); };
