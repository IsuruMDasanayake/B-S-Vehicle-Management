...
const FuelForm = ({ editId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/fuel-entries/${id}`)
        ...
        .catch(() => { toast.error('Failed to load entry'); if (!isModal) navigate('/vehicle/admin/fuel'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/fuel');
    } catch (e) { toast.error(e.response?.data?.message || 'Something went wrong'); }
    finally { setIsLoading(false); }
  };

  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/fuel'); };
