...
const DepartmentForm = ({ editId, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      api.get(`/departments/${id}`).then(({ data }) => reset(data))
        .catch(() => { toast.error('Failed to load'); if (!isModal) navigate('/vehicle/admin/departments'); else onClose?.(); });
    }
  }, [id]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isModal) onSuccess(); else navigate('/vehicle/admin/departments');
    } catch (e) { toast.error(e.response?.data?.message || 'Error'); }
    finally { setIsLoading(false); }
  };

  const handleCancel = () => { if (isModal) onClose?.(); else navigate('/vehicle/admin/departments'); };
